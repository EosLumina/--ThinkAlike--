...
class DocumentationService:
    ...
    def update_api_documentation(self) -> List[str]:
        ...
        router_files = list(
            Path(self.src_dir / "backend/app/routes").glob("*.py"))
        ...
        for router_file in router_files:
            ...
            if endpoints:
                ...
                out_file = self.docs_dir / "architecture" / \
                    "api" / f"api_endpoints_{module_name}.md"
    ...
    def update_model_documentation(self) -> List[str]:
        ...
        model_files = list(
            Path(self.src_dir / "backend/app/models").glob("*.py"))
        ...
        for model_file in model_files:
            ...
            if models:
                ...
                # Determine output file path
                out_file = self.docs_dir / "architecture" / \
                    "database" / f"models_{module_name}.md"
                out_file.parent.mkdir(parents=True, exist_ok=True)
            ...
            for link_text, link_target in matches:
                ...
                if target_filename in filename_mapping:
                    ...
                    if '/' in link_target:
                        ...
                        new_link_target = '/'.join(
                            target_parts[:-1] + [new_filename])
                    else:
    ...
    def generate_api_summary(self) -> str:
        ...
        api_docs = list(Path(self.docs_dir / "architecture" /
                        "api").glob("api_endpoints_*.md"))
    ...
    def _extract_endpoints(self, router_file: Path) -> List[Dict]:
        ...
        for method, path in matches:
            ...
            func_pattern = re.compile(
                r'@router\.{}[\s\S]*?def\s+(\w+)'.format(method))
    ...
    def _extract_response_model(self, content: str, method: str, path: str) -> str:
        ...
        """
        pattern = re.compile(
            r'@router\.{}[\s\S]*?response_model=(\w+)'.format(method))
        match = pattern.search(content)
    ...
    def _extract_models(self, model_file: Path) -> List[Dict]:
        ...
        for model_name in matches:
            ...
            tablename_pattern = re.compile(
                r'class\s+{}.*?__tablename__\s*=\s*[\'"]([^\'"]+)[\'"]'.format(model_name))
            ...
            docstring = docstring_match.group(
                1).strip() if docstring_match else ""
