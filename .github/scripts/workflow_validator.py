...
class WorkflowValidator:
    ...
    def validate_and_fix_workflows(self, file_paths: List[str], fix: bool = False) -> bool:
        ...
        for file_path in file_paths:
            ...
            try:
                ...
                self.add_error(
                    file_path, f"Unexpected error during validation: {e}")
                all_valid = False
        ...
        if self.verbose:
            ...
            print(
                f"Warnings found: {len([w for w in self.warnings if not w[2]])}")
            if fix:
                print(f"Fixes applied: {len(self.fixes)}")
                print(f"Errors fixed: {len([e for e in self.errors if e[2]])}")
                print(
                    f"Warnings fixed: {len([w for w in self.warnings if w[2]])}")
    ...
    def validate_and_fix_workflow(self, file_path: str, fix: bool = False) -> bool:
        ...
        try:
            ...
            if yaml_content is None:
                self.add_error(
                    file_path, "File appears to be empty or invalid YAML")
                is_valid = False
            else:
                ...
                if 'jobs' not in yaml_content and jobs_match:
                    self.log(
                        f"Warning: Found 'jobs:' in content but not in parsed YAML. Possible YAML parsing issue.")
    ...
    def _validate_workflow_structure(self, file_path: str, yaml_content: Dict, raw_content: str) -> bool:
        ...
        if on_match:
            self.log(
                f"Found 'on:' section directly in file content at position {on_match.start()}")
            on_section_found = True

        # If not found, check for 'on': (with quotes)
        if not on_section_found:
            on_match = re.search(r"^\s*'on'\s*:", raw_content, re.MULTILINE)
            if on_match:
                self.log(
                    f"Found 'on:' section (with quotes) in file content at position {on_match.start()}")
                on_section_found = True
        ...
        if on_section_found and 'on' not in yaml_content:
            self.log(
                f"Warning: Found 'on:' in content but not in parsed YAML. Possible YAML parsing issue.")
    ...
    def _validate_jobs_section(self, file_path: str, yaml_content: Dict, raw_content: str) -> bool:
        ...
        if 'jobs' in yaml_content:
            ...
            if not isinstance(jobs, dict) or not jobs:
                self.add_error(
                    file_path, "Jobs section is empty or not a dictionary")
                return False
            ...
            for job_id, job in jobs.items():
                ...
                if not isinstance(job, dict):
                    self.add_error(
                        file_path, f"Job '{job_id}' is not a dictionary")
                    is_valid = False
                    continue
                ...
                if 'runs-on' not in job:
                    self.add_error(
                        file_path, f"Job '{job_id}' missing 'runs-on' attribute")
                    is_valid = False

                # Check for steps
                if 'steps' not in job:
                    self.add_error(
                        file_path, f"Job '{job_id}' missing 'steps' attribute")
                    is_valid = False
                elif not job['steps']:
                    self.add_warning(
                        file_path, f"Job '{job_id}' has empty 'steps' list")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Validate and fix GitHub Actions workflow files.')
    parser.add_argument('--fix', action='store_true',
                        help='Fix issues in workflow files')
    ...
    if not args.files:
        ...
        if os.path.isdir(workflow_dir):
            ...
            print(
                f"❌ Error: Default workflow directory {workflow_dir} not found")
            return 1
