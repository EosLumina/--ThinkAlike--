  ...
  return function (lunr) {
    ...
    TinySegmenter.prototype.segment = function (input) {
      ...
      for (var i = 4; i < seg.length - 3; ++i) {
        ...
        p1 = p2;
        p2 = p3;
        p3 = p; // Correctly update p3 for the next iteration
        word += w4; // Append current character to the word being built
      }
      result.push(word); // Add the last word segment
      return result;
    };

    // Register the plugin with lunr
    lunr.TinySegmenter = new TinySegmenter;
    lunr.Pipeline.registerFunction(lunr.TinySegmenter.segment, 'tinySegmenter');

    // Add Japanese language support
    lunr.jp = function (builder) {
      builder.pipeline.remove(lunr.stemmer);
      builder.pipeline.remove(lunr.stopWordFilter);
      builder.pipeline.add(lunr.jp.stopWordFilter); // Add Japanese stop words if needed
      builder.pipeline.add(lunr.tinySegmenter); // Add the segmenter
      builder.pipeline.add(lunr.stemmer); // Re-add stemmer if applicable after segmentation
      builder.pipeline.add(lunr.trimmer);
    };

    // Example Japanese stop words (add more as needed)
    lunr.jp.stopWords = new lunr.SortedSet;
    lunr.jp.stopWords.add('これ');
    lunr.jp.stopWords.add('それ');
    lunr.jp.stopWords.add('あれ');
    lunr.jp.stopWords.add('この');
    lunr.jp.stopWords.add('その');
    lunr.jp.stopWords.add('あの');
    // ... add more stop words

    lunr.jp.stopWordFilter = lunr.generateStopWordFilter(lunr.jp.stopWords.toArray());

    lunr.Pipeline.registerFunction(lunr.jp.stopWordFilter, 'stopWordFilterJp');

    // Add trimmer function if not already present globally
    if (!lunr.trimmer) {
      lunr.trimmer = function (token) {
        return token.update(function (s, m) {
